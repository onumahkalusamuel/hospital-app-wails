...
import { Pagination, Invoice, Patient } from '@/interfaces'
...
onMounted(async () => { fetchInvoices(), fetchPatient() })
...
<template>
    <div class="page-wrapper">
        ...
        <PageHeader title="Billings" :subtitle="`${patient.lastname} ${patient.firstname} billings`"
            :icon-src="BanknotesIcon">
        </PageHeader>
        <div class="px-[15px] flex justify-between border-t-[1px] border-[#333] py-2">
            <div class="pr-2">
                <ActionButton dark
                    v-on:click="() => $router.push({ name: 'add-invoice', params: { patient_id: patient.id } })"
                    :icon-src="PlusIcon">Add invoice
                </ActionButton>
            </div>
            <div class="flex">
                    ...
                    <div class="table-row" v-for="invoice, i in pagination.rows" :key="invoice.id">
                        ...
                        <div class="cell-data cell-size-1">
                            <router-link class="text-blue-600 hover:underline"
                                :to="{ name: 'view-invoice', params: { id: invoice.id } }">
                        ...
                        <div class="cell-data">
                            <ActionButton outline v-if="user.role == '1'" v-on:click="deletItem(invoice.id)" :icon-src="TrashIcon">
                                Delete</ActionButton>
                        </div>
                    </div>
                </div>
